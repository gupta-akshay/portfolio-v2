...
const QueuePanel: React.FC<QueuePanelProps> = ({
  ...
  const handleDragOver = (e: React.DragEvent, index: number) => {
    ...
    const draggedItem = document.querySelector(`[data-queue-index="${draggedIndex}"]`);
            ...
            <div className="queueTrackItem nowPlayingItem">
              <div className="queueTrackInfo">
                ...
                <div className="queueTrackDetails">
                  <span className="queueTrackTitle">{currentTrack.name || currentTrack.title}</span>
                  <span className="queueTrackArtist">{currentTrack.artist}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="upNextSection">
          ...
          {queueTracks.length > 0 ? (
            <ul className="queueTrackList">
              {queueTracks.map((track, index) => (
                ...
                >
                  ...
                  >
                    <div className="queueTrackDetails">
                      <span className="queueTrackTitle">{track.name || track.title}</span>
            ...
            <div className="emptyQueue">
              ...
              <p className="emptyQueueSubtext">Add tracks to your queue by clicking the "Add to Queue" button on any track.</p>
            </div>
          )}
        </div>
      </div>
