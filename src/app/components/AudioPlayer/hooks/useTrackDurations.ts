 ...
 */
export const useTrackDurations = (tracks: Track[], cloudName: string | undefined) => {
    ...
    const loadTrackDurations = async () => {
      ...
      for (const track of tracks) {
        ...
        try {
          ...
          if (!trackDurations[track.id] && !loadedTracksRef.current.has(track.id)) {
            ...
            await new Promise<void>((resolve, reject) => {
              ...
              const onLoadedMetadata = () => {
                if (hiddenAudioRef.current) {
                  ...
                  console.log(`Loaded duration for ${track.title}: ${hiddenAudioRef.current.duration}s`);
                  resolve();
                }
              };
              
              const onError = (e: ErrorEvent) => {
                console.error(`Error loading metadata for track: ${track.title}`, e);
              ...
              // Add event listeners
              hiddenAudioRef.current.addEventListener('loadedmetadata', onLoadedMetadata, { once: true });
              hiddenAudioRef.current.addEventListener('error', onError as EventListener, { once: true });
              
              // Set a timeout to handle cases where the audio might hang
              const timeoutId = setTimeout(() => {
                console.warn(`Timeout loading metadata for track: ${track.title}`);
          ...
          console.error(`Error loading duration for track ${track.title}:`, error);
